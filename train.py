...
def train_epoch(model, train_loader, optimizer, mfcc_transform, device):
    model.train()
    total_loss = 0.0
    ...
    for batch in tqdm(train_loader):
        ...
        logits = model(mfcc.to(device), logit_lengths)  # (B, T, N)
...
@torch.no_grad()
def evaluate(model, dev_loader, dev_dataset, mfcc_transform, device):
    model.eval()
    total_loss = 0.0
...
def main():
    ...
    while epoch < MAX_EPOCHS:
        ...
        epoch += 1
        train_loss = train_epoch(model, train_loader, optimizer, mfcc_transform, device)
        dev_loss, greedy_cer, beam_cer = evaluate(model, dev_loader, dev_dataset, mfcc_transform, device)
